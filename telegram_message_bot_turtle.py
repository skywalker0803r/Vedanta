...
def main():
    ...
    for symbol in top_symbols:
        print(f"分析 {symbol}...")
        try:
            result = turtle_strategy.get_signals(symbol=symbol, interval='4h', end_time=datetime.now(), limit = 1000).tail(1)
            if result["signal"].values[0] == 1:
                print(f"{symbol} 多單訊號 - 海龜多單")
                long_symbols.append(f"{symbol} (海龜多單)")
            if result["signal"].values[0] == -1:
                print(f"{symbol} 空單訊號 - 海龜空單")
                short_symbols.append(f"{symbol} (海龜空單)")
        except Exception as e:
